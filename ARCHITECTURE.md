ARCHITECTURE.md · v0.6 · 2026-09-26
...
- warm-up nulls: `llm_call.query_id` and `.seed` are null iff `role == "warmup"`;
...
| M2 | data-eval | Vendoring + VENDOR.lock, `scripts/vendor_check.py` (tracked files only), database unpacking and per-file manifest (D5), evalenv, bridge, constraints, aggregate, golden fixtures | M0 |
